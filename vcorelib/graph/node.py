...
class DiGraphNode(_AbstractDiGraphNode):
    ...
    def add_child(self, label: str, node: _T = None, **kwargs) -> _T:
        """
        ...
        node = self.graph.handle_node(label, node=node)
        self.graph.add_edge(self.label, node.label, **kwargs)
        return node

    def add_parent(self, label: str, node: _T = None, **kwargs) -> _T:
        """
        ...
        # Ensure the other node is a vertex in the graph.
        node = self.graph.handle_node(label, node=node)
        self.graph.add_edge(node.label, self.label, **kwargs)
        return node

    def add_parallel(self, label: str, node: _T = None, **kwargs) -> _T:
        """
        ...
        # Ensure the other node is a vertex in the graph.
        node = self.graph.handle_node(label, node=node)
        self.graph.add_parallel(self.label, node.label, **kwargs)
        return node
